/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v5.5)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v5.5_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Consolidated master script including all tables, RLS, and functions.
   Based on v5.4 with critical fixes for Realtime Voting and Invitation Reliability.
   
   KEY CHANGES (v5.5):
   1. Added `citext` extension for case-insensitive email matching.
   2. Changed `trip_members.invited_email` to `citext`.
   3. Added `itinerary_votes` and `trip_members` to `supabase_realtime` publication.
   4. Set `REPLICA IDENTITY FULL` on voting and members for consistent Realtime deltas.
...
create extension if not exists "citext" with schema "public"; -- Added for case-insensitive email
...
create table trip_members (
    ...
    invited_email citext, -- Changed to citext for case-insensitive matching
...
-- Helper 1: Check Membership (Bypasses RLS on trip_members as security definer)
...
-- Helper 2: Check Ownership (Bypasses RLS on trip_members as security definer)
-- REFACTORED in v5.3: Checks trip_members instead of trips to avoid circular policy dependency
...
-- TRIP MEMBERS (FIXED in v5.3 to prevent recursion)
...
create policy "Link self" on trip_members 
for update 
using (
...
-- ITINERARY ATTACHMENTS (New in v5.2)
...
alter publication supabase_realtime add table itinerary_votes; -- NEW: Fixes Voting Refresh
alter publication supabase_realtime add table trip_members;   -- NEW: Fixes Invite Visibility
