/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v5.9)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v5.9_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Consolidated master script including all tables, RLS, and functions.
   Based on v5.8 with fixes for SQL syntax and missing security policies.
   
   KEY CHANGES (v5.9):
   1. FIXED: `drop table if exists user_relationships` syntax error.
   2. FIXED: `citext` extension moved to `extensions` schema.
   3. SECURITY: Added missing RLS policies for `ai_conversations`, `booking_inbox`, etc.
   4. SECURITY: Enabled RLS on `user_relationships`.
...
drop table if exists user_relationships cascade; -- Fixed syntax error here
...
-- Move citext to extensions schema if it exists in public, otherwise create in extensions
do $$
begin
...
create table trip_members (
    ...
    invited_email extensions.citext, -- Using schema qualified name just in case
...
returns trigger language plpgsql security definer set search_path = public as $$
begin
    -- v5.8 Fix: Add ON CONFLICT DO NOTHING to prevent race conditions
...
-- Helper 1: Check Membership (Secure)
...
-- Helper 2: Check Ownership (Secure)
...
alter table user_relationships enable row level security; -- Enforced RLS
...
-- BOOKING INBOX (Added v5.9)
create policy "Manage inbox" on booking_inbox for all using (user_id = auth.uid());

-- PRICE ALERTS (Added v5.9)
create policy "Manage alerts" on price_alerts for all using (user_id = auth.uid());

-- PLACE KNOWLEDGE (Added v5.9)
create policy "Read places" on place_knowledge for select using (true);
create policy "Manage places" on place_knowledge for all using (auth.uid() IN (SELECT id FROM profiles WHERE subscription_tier = 'pro')); -- Example restriction

-- USER RELATIONSHIPS (Added v5.9)
