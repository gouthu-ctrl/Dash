/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v6.2)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v6.2_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Fixes "new row violates row-level security policy for table profiles".
   Includes all previous fixes (v6.1 recursion fix, v6.0 regressions).
   
   KEY CHANGES (v6.2):
   - POLICY "Insert self": Added missing INSERT policy for `profiles` table.
     This allows `upsert` operations from the client to succeed.
   
   KEY CHANGES (v6.1 Recap):
   - Fixed "Infinite Recursion" in trips/members policies using security definer functions.
...
-- PROFILES (Fixed: Added INSERT policy)
create policy "Public profiles" on profiles for select using (true);
create policy "Update self" on profiles for update using (auth.uid() = id);
create policy "Insert self" on profiles for insert with check (auth.uid() = id); -- FIX for RLS v6.2
