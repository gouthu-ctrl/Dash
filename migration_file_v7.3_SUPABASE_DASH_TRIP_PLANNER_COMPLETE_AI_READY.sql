/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v7.3)
   ========================================================================
   
   PATH: migration_file_v7.3_SUPABASE_DASH_TRIP_PLANNER_COMPLETE_AI_READY.sql
   
   OVERVIEW:
   This is the DEFINITIVE, COMPLETE schema definition for the Dash Trip Planner app.
   It replaces all previous migration files (v7.2, v7.0, etc.).
   
   KEY CHANGES (v7.3):
   - Merged v7.2 (Base comprehensive schema + `get_user_stats`).
   - Added specific optimizations for Hybrid AI features (Phase 33/34):
     - Added indices on `ai_conversations` for fast history retrieval.
     - Added `match_places_v2` logic (placeholder) for future vector search.
   - Retains all RLS, Triggers, and Realtime config.
   
   EXECUTION:
   Run in Supabase SQL Editor.
   WARNING: This script DROPS EXISTING TABLES. Back up data if running on production.
========================================================================
*/

-- 1. CLEANUP (Drop everything to ensure fresh state)
...
drop function if exists public.match_places_v2(vector, int, jsonb) cascade;
...
drop table if exists ai_conversations cascade; -- AI Table
...
-- [NEW v7.3] Optimize fetching conversations
create index idx_ai_conversations_trip_id on ai_conversations(trip_id);
...
declare
    _result json;
begin
    -- The table constraint check(vote_type in ('up', 'down')) will automatically 
    -- raise an error if an invalid type is passed.
...
-- [AI VECTOR SEARCH - v7.3 Preparedness]
-- Call this via RPC if you switch back to Embedding/Vector search strategy
create or replace function public.match_places_v2(
...
begin
    return query(
        select
            place_knowledge.id,
            place_knowledge.place_name,
            place_knowledge.description,
            1 - (place_knowledge.embedding <=> query_embedding) as similarity
        from place_knowledge
        where 1 - (place_knowledge.embedding <=> query_embedding) > match_threshold
