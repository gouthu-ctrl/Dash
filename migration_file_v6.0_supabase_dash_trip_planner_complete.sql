/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v6.0)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v6.0_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Definitive master script. Fixes all known regressions:
   1. Trip Creation Visibility (Trigger hardening).
   2. Invitation Rejection Logic (RLS updates).
   3. Voting Sync (Table & Policy stability).
   4. Syntax errors from v5.9 fixed.
   
   KEY CHANGES (v6.0):
   - POLICY "View trips": Now explicitly excludes trips where user status is 'declined'.
   - POLICY "View members": Allows viewing 'declined' members so team sees who rejected.
   - TRIGGER `handle_new_trip`: Uses `ON CONFLICT DO NOTHING` and `return new`.
   - TABLE `user_relationships`: Fixed syntax error in drop/create.
...
-- 1. CLEANUP (Drop in specific order to avoid cascade issues)
...
returns trigger language plpgsql security definer set search_path = public as $$
begin
    -- CRITICAL FIX: ON CONFLICT DO NOTHING to prevent race conditions during heavy load
...
-- Helper 1: Check Membership (Optimized)
...
returns boolean language sql security definer set search_path = public, auth as $$
    select exists (
        ...
        and status = 'accepted' -- Only Accepted members count as 'active' members for permissions
        and (
            _req_role = 'viewer'
...
-- TRIPS (Updated for Invites/Rejection)
create policy "View trips" on trips for select using (
    created_by = auth.uid()             
    or (
        -- Member view: Must NOT be declined
        exists (
            select 1 from trip_members 
            where trip_id = id 
            and user_id = auth.uid() 
...
create policy "View members" on trip_members for select using (
    ...
    -- Members can see ALL other members (including declined) to know who bailed
    or exists ( 
...
create policy "Manage self status" on trip_members for update using (
    ...
    status in ('accepted', 'declined') -- Allow rejecting
);
