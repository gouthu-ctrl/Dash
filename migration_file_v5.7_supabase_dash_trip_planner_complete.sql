/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v5.7)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v5.7_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Consolidated master script including all tables, RLS, and functions.
   Based on v5.6 with critical fixes for Pending Trip Visibility.
   
   KEY CHANGES (v5.7):
   1. Added "View pending trips" policy to `trips`.
      (Allows users to see the trip they are invited to before accepting).
   2. Added "Manage self status" policy to `trip_members`.
      (Allows users to update their own status to 'accepted' or 'declined').
   3. Updated `is_trip_member` and `View trips` policies to handle pending states gracefully.
...
-- Helper 3: Check if user has a pending invitation to this trip
...
create policy "View trips" on trips for select using (
    ...
    or has_pending_invite(id) -- Added in v5.7: User can see meta-data of trips they are invited to
...
create policy "View members" on trip_members for select using (
    user_id = auth.uid()
    or invited_email = auth.jwt() ->> 'email' -- Added in v5.7: See own pending record
