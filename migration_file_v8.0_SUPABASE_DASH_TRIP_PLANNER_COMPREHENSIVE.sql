/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v8.0)
   ========================================================================
   
   PATH: migration_file_v8.0_SUPABASE_DASH_TRIP_PLANNER_COMPREHENSIVE.sql
   
   OVERVIEW:
   This is the DEFINITIVE, COMPREHENSIVE schema definition for the Dash Trip Planner app.
   It consolidates all previous migrations (v7.x, v6.x, etc.) and adds support for:
   - AI Trip Planning (Conversations & suggestions)
   - Real-time collaboration
   - User statistics calculation (RPC)
   - Advanced RLS security policies
   
   KEY FEATURES:
   1. AI Strategy Support: Tables for managing AI-generated itineraries.
   2. Real-time: Multi-user sync for trips, itinerary, and voting.
   3. Document Vault: Encrypted storage of travel documents.
   4. Expense Management: Group split and settlement tracking.
...
-- used by AI generation strategy for proposed plans
create table itinerary_items (
    id uuid default uuid_generate_v4() primary key,
    trip_id uuid references trips(id) on delete cascade,
    type text not null, -- e.g., 'activity', 'eat', 'stay', 'flight'
...
-- ITINERARY VOTES: Real-time decision making
...
-- ITINERARY ATTACHMENTS: Files related to items (e.g., tickets)
...
-- DOCUMENT VAULT: Secure storage for passports, etc.
...
-- BOOKING INBOX: Parsing emails from providers
...
-- TRIP CHAT: Human-to-Human real-time chat
...
-- BUCKETS: Storage initialization
...
begin
    ...
    -- Link any old invites to the new user
...
-- Auto-accept owner as member on trip creation
...
-- User Stats Calculation RPC (Optimized counting)
