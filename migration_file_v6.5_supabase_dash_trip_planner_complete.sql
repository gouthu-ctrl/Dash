/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v6.5)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v6.5_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   This is the DEFINITIVE, COMPLETE schema definition for the Dash Trip Planner app.
   It replaces all previous migration files (v1.0 - v6.4).
   
   KEY FIXES IN v6.5:
   1. VOTING PERSISTENCE:
      - Replaced generic "ALL" policy for `itinerary_votes` with granular 
        INSERT, UPDATE, DELETE policies. This resolves issues with Supabase's `upsert`
        behavior failing when a policy is ambiguous.
   2. STRICT ITINERARY SECURITY:
      - `itinerary_items` modifications are STRICTLY limited to 'owner' and 'editor' roles.
      - Explicitly prevents Viewers from inserting, updating, or deleting items via API.
   3. ROLE ENFORCEMENT:
      - Includes the `trg_protect_member_role` trigger to prevent non-owners from 
        changing member roles.
   4. DATA INTEGRITY:
      - Includes `link_user_identity` trigger to auto-connect invites.
      - Ensures unique constraints on voting.

   EXECUTION:
   Run in Supabase SQL Editor. This will reset/ensure the schema is in the correct state.
========================================================================
*/
...
-- ITINERARY ITEMS (Strict RBAC - Fixed)
create policy "Items view" on itinerary_items for select using (
    is_trip_owner(trip_id)
    or is_trip_member(trip_id, 'viewer')
);

-- Only Editors/Owners can Insert/Update/Delete (Separate policies for clarity)
create policy "Items insert" on itinerary_items for insert with check (
    is_trip_editor(trip_id)
);
create policy "Items update" on itinerary_items for update using (
    is_trip_editor(trip_id)
);
create policy "Items delete" on itinerary_items for delete using (
    is_trip_editor(trip_id)
);
...
-- ITINERARY VOTES (Fixing Persistence/Update - Split Policies)
-- A. SELECT
create policy "Vote select" on itinerary_votes for select using (true);

-- B. INSERT
create policy "Vote insert" on itinerary_votes for insert with check (
    user_id = auth.uid()
);

-- C. UPDATE
create policy "Vote update" on itinerary_votes for update using (
    user_id = auth.uid()
);

-- D. DELETE
create policy "Vote delete" on itinerary_votes for delete using (
    user_id = auth.uid()
);
...
create policy "Manage splits" on expense_splits for all using (
    exists (select 1 from expenses where id = expense_id and is_trip_editor(trip_id))
);
