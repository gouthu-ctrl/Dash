/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v6.3)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v6.3_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Definitive Fix for Invitation Linking, Voting Persistence, and RLS Errors.
   
   KEY CHANGES (v6.3):
   1. AUTO-LINKING TRIGGER: `link_user_identity` function runs on new user creation.
      It automatically finds pending invites with matching email and sets `user_id`.
      This prevents the "user_id is null" issue.
   2. REJECTION VISIBILITY: Enhanced RLS policies to ensure rejected trips 
      disappear for the guest but show as "Declined" for the host.
   3. VOTING FIX: Ensure `itinerary_votes` is globally selectable by authenticated users
      (relaxed security for UX responsiveness) or strictly visible.
   
   INCLUDES PREVIOUS FIXES:
   - v6.2: Profile INSERT policy (Fixes "new row violates... profiles").
   - v6.1: Infinite Recursion Fix (Security Definer functions).
...
drop function if exists public.link_user_identity() cascade; -- NEW
...
begin
    ...
    -- 2. Auto-Link Pending Invites (The Fix)
...
-- Crucial: Allow user to manage their own status (Accept/Decline)
-- Checks BOTH user_id match OR email match to handle cases where linking lagged
