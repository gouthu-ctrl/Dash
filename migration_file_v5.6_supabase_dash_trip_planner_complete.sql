/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v5.6)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v5.6_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Consolidated master script including all tables, RLS, and functions.
   Based on v5.5 with critical fix for Invitation Selection visibility.
   
   KEY CHANGES (v5.6):
   1. Added "View own invitation" policy to `trip_members` for SELECT.
      (Fixes the issue where users couldn't see their own record to update it).
   2. Ensured `REPLICA IDENTITY FULL` on all Realtime tables.
   3. Maintained `citext` for case-insensitive email matching.
...
create policy "View own invitation" on trip_members for select using (
