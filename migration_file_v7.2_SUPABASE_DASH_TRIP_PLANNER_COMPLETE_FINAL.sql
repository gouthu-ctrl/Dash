/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v7.2)
   ========================================================================
   
   PATH: migration_file_v7.2_SUPABASE_DASH_TRIP_PLANNER_COMPLETE_FINAL.sql
   
   OVERVIEW:
   This is the DEFINITIVE, COMPLETE schema definition for the Dash Trip Planner app.
   It replaces all previous migration files (v7.0, v6.x, etc.).
   
   KEY CHANGES (v7.2):
   - Merged v7.0 (Base comprehensive schema).
   - Added `get_user_stats` RPC function (Phase 29) for calculating trip/place counts efficiently.
   - Includes all previous RLS, Triggers, and Extensions.
   - Full 'DROP AND RECREATE' strategy for clean deployment (Use with caution on prod).
...
drop function if exists public.get_user_stats(uuid) cascade; -- v7.2 Addition
...
create table profiles (
    ...
    theme_preference text default 'system', -- v7.0: Theme setting
    language_preference text default 'en',   -- v7.0: Language setting
