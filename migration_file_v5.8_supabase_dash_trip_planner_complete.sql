/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v5.8)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v5.8_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   Consolidated master script including all tables, RLS, and functions.
   Based on v5.7 with hardened RLS and Triggers for reliability.
   
   KEY CHANGES (v5.8):
   1. `handle_new_trip` uses `ON CONFLICT DO NOTHING` to prevent race conditions.
   2. `View members` policy explicitly allows viewing 'declined' members if you are an accepted member.
   3. `View trips` policy re-verified to strictly allow owners.
...
drop user_relationships cascade;
...
create policy "View trips" on trips for select using (
    created_by = auth.uid()             -- v5.8: Guaranteed access for creator
    or is_trip_member(id, 'viewer')     -- Access for accepted members
    or has_pending_invite(id)           -- Access to see invite card
...
create policy "View members" on trip_members for select using (
    ...
    or is_trip_member(trip_id, 'viewer') -- v5.8: Viewers can see ALL members (including declined)
);
