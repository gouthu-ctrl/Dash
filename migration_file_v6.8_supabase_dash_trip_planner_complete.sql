/*
========================================================================
   DASH TRIP PLANNER APP - MASTER MIGRATION SCRIPT (v6.8)
========================================================================
   
   PATH: d:\StudioProjects\Dash\migration_file_v6.8_supabase_dash_trip_planner_complete.sql
   
   OVERVIEW:
   This is the DEFINITIVE, COMPLETE schema definition for the Dash Trip Planner app.
   It replaces all previous migration files (v6.7 and earlier).
   
   KEY CHANGES (v6.8):
   - REMOVED 'heart'/'favorite' voting option Support.
   - ADDED check constraint on `itinerary_votes` to allow only 'up' or 'down'.
   - Retains `cast_vote` RPC function for reliable persistence.
...
create table itinerary_votes (
    ...
    user_id uuid references profiles(id) on delete cascade,
    vote_type text check (vote_type in ('up', 'down')), -- v6.8: Constraint strictly enforces Up/Down
...
-- [VOTING RPC FUNCTION - v6.8: Supports Constraint]
...
-- ITINERARY VOTES (Separate Policies + RPC used preferred)
